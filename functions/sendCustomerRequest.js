...
requestFunction.sendMessageToDriver = async function (vehicleID, ticketID, passengerData) {
    const getDeviceTokenPromise = admin.firestore().collection('fcmTokens').doc(vehicleID).get().then((snapshot) => {
...
requestFunction.sendMessageToUser = async function (userID, assignedVehicle, message) {
    const getDeviceTokenPromise = admin.firestore().collection('fcmTokens').doc(userID).get().then((snapshot) => {
        return snapshot.data().fcmToken;
    });
    ...
    if (assignedVehicle != null) {
        ...
        var driver;
        await admin.firestore().collection('Drivers').doc(assignedVehicle.driver).get().then((doc) => {
            if (doc.exists) {
                driver = doc.data();
            }
        });
        ...
        var position;
        await admin.firestore().collection('PrivateVehicles').doc(assignedVehicle.registration).get().then((doc) => {
            if (doc.exists) {
...
requestFunction.sendCancelNotification = async function (recieverId) {
    const getDeviceTokenPromise = admin.firestore().collection('fcmTokens').doc(recieverId).get().then((snapshot) => {
        return snapshot.data().fcmToken;
    });
